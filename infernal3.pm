...
sub next_hit {
  ...
  while (my $aline=$self->_readline()) {
    ...
    if ($aline eq '') {next;}
    if ($aline=~/CYKDivideAndConquer/) {next;}
    ...
    my $prefix_space;
    if ($std_seq=~/^(\s+)([\d|\-]+)(\s)(.*?)\s([\d|\-]+)\s+$/) {
       my $frag_start=$2; my $frag_end=$5;
       my $temp_seq = $4;
    ...
    if ($target_seq=~/^(\s+)([\d|\-]+)(\s)(.*?)\s([\d|\-]+)\s+$/) {
       my $target_frag_start = $2;
       my $target_frag_end   = $5;
       my $temp_seq          = $4;

       if (!defined($onehit{'target_start'})) {
          if ($target_frag_start=~/\d+/) {
             $onehit{'target_start'}=$target_frag_start;
          }
       }
       if ($target_frag_end=~/\d+/) {
          $onehit{'target_end'} = $target_frag_end;
       }
       $onehit{'target_seq'}.=$temp_seq;
    }
  }

  #print $onehit{'struc'}, "\n";
  #print $onehit{'std_seq'}, "\n";
  #print $onehit{'CMstring'}, "\n";
  #print $onehit{'target_seq'}, "\n";
