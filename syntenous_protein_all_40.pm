package syntenous_protein_all_40;
use strict;
...
sub new {
  ...
  my ($conf_file, $host, $dbuser, $port, $dbname, $assembly1, $assembly2, $species1, $species2, $no_all_homologs) = 
  $self->_rearrange([qw(CONF_FILE HOST DBUSER PORT DBNAME ASSEMBLY1 ASSEMBLY2 SPECIES1 SPECIES2 NO_ALL_HOMOLOGS)], @args);
  ...
  $self->{'memberadp'}     = $db->get_MemberAdaptor;
  $self->{'MethodLinkSpeciesSetAdaptor'} = $db->get_MethodLinkSpeciesSetAdaptor();
  ...
  warn "pre fetching MethodLinkSpeciesSet\n";
  my $mlss = $self->{'MethodLinkSpeciesSetAdaptor'}->fetch_by_method_link_type_genome_db_ids(
                                             "ENSEMBL_HOMOLOGUES", 
                                             [$self->{'genome_db_id1'}, 
                                             $self->{'genome_db_id2'}]);
  ...
  my $homologyListRef = $self->{'homoloadp'}->fetch_all_by_MethodLinkSpeciesSet_ext($mlss);
  ...
  #print $ordered_raw_genome_db_ranges_by_db1[0], "\n";
  $self->{'ordered_raw_genome_db_ranges_by_db2'} = \@ordered_raw_genome_db_ranges_by_db2;

  return $self; # success - we hope!
}

sub getAllRelatedProteinsByDB1 {
  my $self = shift;
  my $aDBR = shift;

  if (!$aDBR->isa("genomedbrange")) {
     return undef, undef;
  }

  my $slice = getSliceByGenomeDBRange($self->{'sliceadaptor1'}, $aDBR);
  if (!defined($slice)) {
     die "something wrong in getAllRelatedProteinsByDB1 of syntenous_protein_all_40.pm\n";
  }

  my $gene_list_ref = $self->{'genead1'}->fetch_all_by_Slice($slice);
  my @db2_genes_list = ();
  
  foreach my $single_gene (@$gene_list_ref) {
     my $aMember = $self->{'memberadp'}->fetch_by_source_stable_id(
                               "ENSEMBLGENE", 
                               $single_gene->stable_id);
     if (!defined($aMember)) {next;}
     my $homolog_list = $self->{'homoloadp'}->fetch_all_by_Member_MethodLinkSpeciesSet(
                               $aMember, 
                               $self->{'mlss'});

     foreach my $single_homolog (@$homolog_list) {
	...
	if ($homologcount ==2 ) {
	   ...
	   my @syntenous_genes  = ($members_hash_ref->{$self->{'genome_db_id1'}}->get_Gene, 
           ...
           my $a_db2_homologue = $syntenous_genes[1];
           $a_db2_homologue->{'db1_gene'} = $single_gene;
           push(@db2_genes_list, $a_db2_homologue);
	} # end of if ($homologcount == 2)
     } # end of foreach my $single_homolog
  } # end of foreach my $single_gene
  return $gene_list_ref, \@db2_genes_list;
}
...
sub getBoundaryByDB1_ext {
  ...
  if (!$genomedbrange->isa("genomedbrange")) {return undef, undef, undef, undef, 1;}
  my $ordered_raw_genome_db_ranges_by_db1_ref = $self->{'ordered_raw_genome_db_ranges_by_db1'};


  warn "pre dividefinder\n";
  my $loweridx  = dividefinder(0,              scalar(@$ordered_raw_genome_db_ranges_by_db1_ref)-1, 
                              $genomedbrange, $ordered_raw_genome_db_ranges_by_db1_ref);
  warn "post dividefinder\n";
  my ($pLowerIdx, $pUpperIdx) = $self->getDB1BoundariesIdx($genomedbrange, $loweridx, $ignoreIntervenSingleton);
  
  warn "post getDB1BoundariesIdx\n";
  #my $ori_slice = $self->{'sliceadaptor1'}->fetch_by_region($genomedbrange->getSeqLevel, 
  #                                                          $genomedbrange->getSeqRegion, 
  #                                                          $genomedbrange->getPos1, 
  #                                                          $genomedbrange->getPos2, 
...
sub getBoundSyntenicProteinsByDB1 {
  my $self= shift;
  my $genomedbrange = shift;

  if (!$genomedbrange->isa("genomedbrange")) {return undef;}
  my $ordered_raw_genome_db_ranges_by_db1_ref = $self->{'ordered_raw_genome_db_ranges_by_db1'};
  ...
  my $loweridx  = dividefinder(0,              scalar(@$ordered_raw_genome_db_ranges_by_db1_ref)-1, 
                              $genomedbrange, $ordered_raw_genome_db_ranges_by_db1_ref);
  warn "post dividefinder\n";
  my ($pLowerIdx, $pUpperIdx) = $self->getDB1BoundIdices($genomedbrange, $loweridx);
  
  warn "post getDB1BoundIdices\n";
  
  my @res = ();
  for (my $i = $pLowerIdx; $i <= $pUpperIdx; $i++) {
     push(@res, $self->{'homolog_pairs_ref1'}->{$ordered_raw_genome_db_ranges_by_db1_ref->[$i]});
  }
...
sub dividefinder {
  ...
  if ($upper - $lower == 1) {
     if ($genomedbrange->compareRaw($genomedbrangesRef->[$upper]) > 0) {
        #print $genomedbrangesRef->[$upper], "\n";
        return $upper+1;
     } elsif ($genomedbrange->compareRaw($genomedbrangesRef->[$lower]) < 0) {
        #print $genomedbrangesRef->[$lower], "\n";
        return $lower-1;
     } else {
        return $lower;
     }
  ...
  #print $genomedbrangesRef->[$lower], "\n";
  #print $genomedbrangesRef->[$middle], "\n";
  #print $genomedbrangesRef->[$upper], "\n";
...
sub getDB1BoundIdices {
  my $self = shift;
  my $ori_gdbr = shift;
  my $lowerIdx = shift;
  
  my ($newLowerIdx, $newUpperIdx) = ($lowerIdx + 1, $lowerIdx + 1);
  my $ori_db1_chr = $ori_gdbr->getSeqRegion();
...
sub getUBRHs {
  ...
  my $returnHashRef1 = {};
  my $returnHashRef2 = {};
  #print "The count is ", scalar(@$homology_list_ref), "\n";
  foreach my $single_homolog (@$homology_list_ref) {
        #print $single_homolog->description, "\n"; 
        #between orthologous one2one........ apparent ortholog...
	if ($single_homolog->description ne "ortholog_one2one") {next;}
	my @homologmemattrs = @{$single_homolog->get_Member_Attribute_by_source("ENSEMBLGENE")};
	my $homologcount = scalar(@homologmemattrs);
        #print $homologcount, "\n";
	if ($homologcount ==2 ) {
		...
		$returnHashRef1->{$syntenous_genes[0]->feature_Slice->name} = \@syntenous_genes;
                #print $syntenous_genes[0]->feature_Slice->name, "\n";
                #exit;
